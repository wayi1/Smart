Compile and Run:
1. To compile the file, C++11 should be supported. 

2. Before run any application, check if the OpenMP version is newer than 3.1.
...
3. Since both Scheduler and User Application Classes are templcate classes, if
any header file is modified, "make clean" is required before recompilation.

Description:
If the output layout is fixed, and the output array can be allocated in
advance, then feed the array address to the run/run2 function.
Otherwise, the user needs to transform a global combination map to a final
output manually. Global combination map is combination_map_ on the master node,
and it can be retrieved by calling get_combination_map.
...
To improve the efficiency by supporting early emission of reduction object, 
in the RedObj class, the trigger function should be overwritten.
