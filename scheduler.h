...
class Scheduler {
 public:
  ...
  // The buf_size can be set at most equal to total_len, which results to only
  ...
  void run(const In* data, size_t total_len, size_t buf_size, Out* out, size_t out_len, MAPPING_MODE_T mode);
  ...
  /*
   * Internal APIs for Hacking the Internal.
   */
  // Set up data processing by binding input data.
...
template <class In, class Out>
void Scheduler<In, Out>::run(const In* data, size_t total_len, size_t buf_size, Out* out, size_t out_len, MAPPING_MODE_T mode) {
...
template <class In, class Out>
void Scheduler<In, Out>::run(const In* data, size_t total_len, Out* out, size_t out_len) {
	run(data, total_len, total_len, out, out_len, GEN_ONE_KEY_PER_CHUNK);
}

template <class In, class Out>
void Scheduler<In, Out>::run2(const In* data, size_t total_len, Out* out, size_t out_len) {
	run(data, total_len, total_len, out, out_len, GEN_KEYS_PER_CHUNK);
}
